  ...
  describe '#create' do
    context 'invalid credentials' do
      it 'should present the login page when given an invalid username' do
        post :create, params: { session: { email: invalid_email, password: valid_password } }
      ...
      it 'should present the login page when given an invalid password' do
        post :create, params: { session: { email: valid_email, password: invalid_password } }
        expect(response).to render_template(:new)
        expect(flash[:danger]).to eq('Invalid email/password combination')
      end

      it 'should present the login page for a completely non-existent user' do
        post :create, params: { session: { email: 'fake@email.com', password: '12345' } }
        expect(response).to render_template(:new)
        expect(flash[:danger]).to eq('Invalid email/password combination')
      end
    end
    ...
    context 'valid credentials' do
      before(:each) do
        post :create, params: { session: { email: valid_email, password: valid_password } }
      end
